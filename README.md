...
API RESTful em Node.js/Express para gerenciar usu�rios com MySQL. Suporta execu��o local ou via Docker Compose.
...
src/
  ...
  routes/users_routes.js# Rotas CRUD de usu�rios
docker-compose.yml      # Orquestra��o Node + MySQL
.env                    # Vari�veis de ambiente (exemplo inclu�do)
package.json
```
...
MYSQL_CONN_LIMIT=10
```
> No Compose, `MYSQL_HOST` � sobrescrito para `db` (nome do servi�o do MySQL).

## Como rodar

### Local (sem Docker)
1. Tenha um MySQL acess�vel com os valores do `.env`.
...
3. Suba a API:
   ...
   npm start
   ```
4. API dispon�vel em `http://localhost:${PORT}` (padr�o `3001`).
...
C�digos de status: `201` criado, `400` requisi��o inv�lida, `404` n�o encontrado, `500` erro interno.
...
- Produ��o: use senhas fortes e limite de conex�es (`MYSQL_CONN_LIMIT`), habilite CORS/helmet/rate-limit.
- DB: adicione restri��o `UNIQUE` em `user_email` para evitar duplicatas.
- Valida��o: considere schema validation (Joi/Zod/Yup) e middleware global de erros.
